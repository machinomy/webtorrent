    ...
    wire.mc_payment_handshake.on('send_secret', function (data) {
      ...
      if ('meta' in data && 'token' in data && 'price' in data) {
        ...
        get.concat(request, function (err, res, data) {
          if (err) throw err
          if (res.statusCode === 200) {
            wire.mc_payment_handshake.sendApprove()
          } else {
            wire.mc_payment_handshake.sendReject()
        ...
        if (window.pieceBusy[index] === undefined) {
          ...
          vynos.ready().then(wallet => {
            wallet.buy(receiver, amount, gateway, meta, undefined, window.piecePrice * window.pieces).then(result => {
              ...
              window.alreadyPaying = false
              window.channelOpenError = true
              console.error(err)
              if (err.message && canBeHandled(err.message)) {
                showVynosNotification('Not enough funds, please refill the wallet', 10);
              }
              return
            })
          })
