...
Torrent.prototype._request = function (wire, index, hotswap) {
  ...
  wire.request(index, chunkOffset, chunkLength, function onChunk (err, chunk) {
    ...
    // if (wire.peerEthereumAddress === undefined) {
    //   if (window.config.ONLY_LOCAL_PEERS) {
    //     // Need to run yarn watch-only-local-peers instead of yarn watch
    //     wire.destroy()
    //     return
    //   }
    // }
    ...
    if (window.config.ONLY_LOCAL_PEERS != true) {
      sha1(buf, function (hash) {
        ...
        const promiseListChannels = wallet.listChannels()
        promiseListChannels.then((listOfChannels)=>{
          //for (let item in )
        });

        promiseListChannels.catch(error => {
          console.error(error)
        })
        if (wire.peerEthereumAddress !== undefined) {
          ...
          if (window.pieceBusy[index] === undefined) {
            ...
            // const vynosBuyPromise = wallet.buy('0x508e6569182e0aab365cb9e91f7638fbe34c82fe',
            //     window.piecePrice,
            //     window.hubURL,
            //     window.videoTitle + ' - Piece #' + index + ' - ' + Date.now())
            //
            // console.log('Meta: ' + window.videoTitle + ' - Piece #' + index + ' - ' + Date.now())
            // // const vynosBuyPromise = wallet.buy( wire.peerEthereumAddress,
            // //     window.piecePrice,
            // //     window.hubURL,
            // //     window.videoTitle + ' - Piece #' + index)
            // vynosBuyPromise.catch(error => {
            //   console.error(error)
            // })

            //vynosBuyPromise.then((vynosBuyResponse) => {
            //  console.log('#########################################')
              //console.log(window.videoTitle + ' - Piece #' + index)
              //console.log('ChannelID: ' + vynosBuyResponse.channelId)
             // console.log('Token: ' + vynosBuyResponse.token)
              //console.log('#########################################')

              // vynos.ready().then(wallet => {
              //   for (let ch in wallet.listChannels()) {
              //     wallet.closeChannel(ch.channelId)
              //   }
              // })
