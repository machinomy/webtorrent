...
Torrent.prototype._onWire = function (wire, addr) {
  ...
  if (typeof mc_payment_handshake === 'function') {
    ...
    wire.mc_ph.on('got_address', function (address) {
      self._debug('mc_payment_handshake: received ethereum address: %s (from %s)', address, wire.addr)
      self._peers[wire.addr].ethereumAddress = address
    })
  }
