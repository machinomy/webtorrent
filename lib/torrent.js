...
if (this.paused != true) {
  if (window.chunksDownloaded - window.chunksPayed > 0) {
    window.chunksPayed++
    console.log('Inside IF')
  }
  window.chunksDownloaded++
  console.log('Chunk payed REQ: ' + window.chunksPayed)
...
Torrent.prototype.pause = function () {
  ...
  window.chunksPayed = Math.max(window.chunksDownloaded, window.chunksPayed)
  window.chunksDownloaded = 0
  document.getElementById('main-video').pause()
  window.currentTime = document.getElementById('main-video').currentTime
  console.log('Paused when $'+window.chunksPayed*window.chunkCost+' was payed')
...
Torrent.prototype.resume = function () {
  ...
  this.paused = false
  this._drain()
  //self = new Torrent(self._torrentId, self.client, self._opts)
  client.remove(self._torrentId, (torrent2remove) => {
    client.add(self._torrentId, (torrent) => {
      window.torrent = torrent
      window.file = torrent.files.find(function (file) {
        return file.name.endsWith('.mp4')
      })

      Array.prototype.slice.call(document.getElementById('main-video')).forEach(
          function(item) {
            item.remove()
          });
      document.getElementsByTagName('body')[0].innerHTML += '<video id="main-video"></video>'
      ...
      window.file.renderTo('#main-video')

      console.log('Downloading is resumed at $'+window.chunksPayed*window.chunkCost)
