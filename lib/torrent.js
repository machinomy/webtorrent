...
var mc_payment_handshake = require('mc_payment_handshake')
...
function Torrent (torrentId, client, opts) {
  ...
  window.channelIsOpened = false
  window.channelOpenError = false
  window.openingInProgress = false
}
...
Torrent.prototype._onWire = function (wire, addr) {
  ...
  if (typeof mc_payment_handshake === 'function') {
    wire.use(mc_payment_handshake(wire))
    wire.mc_payment_handshake.on('mc_payment_handshake', function (data) {
      // Opposite ethereum address is placed in field 'data.peerEthereumAddress'
      wire.peerEthereumAddress = data.peerEthereumAddress
    })
    wire.mc_payment_handshake.on('mc_payment_handshake_not_supported', function (data) {
      if (window.config.ONLY_LOCAL_PEERS) {
        // Need to run yarn watch-only-local-peers for popchest instead of yarn watch
        wire.destroy()
      }
    })
    wire.mc_payment_handshake.on('send_secret', function (data) {
      wire.secret = data
      if ('meta' in data && 'token' in data && 'price' in data) {
        let request = window.hubURLVerify + '?meta=' + data.meta + '&token=' + data.token + '&price=' + data.price
        get.concat(request, function (err, res, data) {
          if (err) throw err
          if (res.statusCode === 200) {
            wire.mc_payment_handshake.sendApprove(wire.secret)
          } else {
            wire.mc_payment_handshake.sendReject(wire.secret)
            console.log('Got ' + res.statusCode + ' from /verify. Block piece.')
          }
        })
      } else {
        wire.mc_payment_handshake.sendReject(wire.secret)
        console.error('Wrong data object received from peer. It need to have fields meta, token and price.')
        console.error('data:')
        console.error(data)
      }
    })

    wire.mc_payment_handshake.on('send_approve', function (data) {
      console.log('CATCH APPROVE ON '+data.meta)

      window.alreadyPaying = false
      /// window.needToPay -= amount
      /// index = parseInt(arr[0])
      window.channelIsOpened = true
      window.openingInProgress = false
    })

    wire.mc_payment_handshake.on('send_reject', function (data) {
      console.log('CATCH REJECT ON '+data.meta)
    })
  }
  ...
  if (self.metadata) {
    process.nextTick(function () {
      ...
      // if (wire.peerEthereumAddress === undefined) {
      //   if (window.config.ONLY_LOCAL_PEERS) {
      //     // Need to run yarn watch-only-local-peers for popchest instead of yarn watch
      //     return wire.destroy()
      //   }
...
Torrent.prototype._request = function (wire, index, hotswap) {
  if (window._pauseReading) return
  var self = this
  ...
  wire.request(index, chunkOffset, chunkLength, function onChunk (err, chunk) {
    if (self.destroyed) return
    // if (wire.peerEthereumAddress === undefined && window.config.ONLY_LOCAL_PEERS) return

    if (wire.peerEthereumAddress === undefined && window.config.ONLY_LOCAL_PEERS) {
      wire.destroy()
      return
    }
    ...
    // if (this.paused != true && window.nowPlaying) {
    //   if (window.pieceDownloaded - window.piecePayed > 0) {
    //     window.piecePayed++
    //   }
    //   window.pieceDownloaded++
    //   window.piece = piece
    // }

    // TODO: Optimize IF with sha1
    if (window.config.ONLY_LOCAL_PEERS != true) {
      if (this.paused != true && window.nowPlaying) {
        if (window.pieceDownloaded - window.piecePayed > 0) {
          window.piecePayed++
        }
      ...
      if (wire.peerEthereumAddress !== undefined) {
        // console.log('peerEthereumAddress:')
        // console.log(wire.peerEthereumAddress)

        // if (window.pieceBusy[index] === undefined) {
        window.counter++
        window.pieceBusy[index] = true;
        // 0x508e6569182e0aab365cb9e91f7638fbe34c82fe - hub.machinomy.com
        // 0x858307b19fB2d04945bE60a71365Ac5E7B5F3b20 - wallet

        let title = window.videoTitle
        let receiver = window.receiver
        let amount = window.piecePrice
        let gateway = window.hubURL
        let meta =  index + ''
        window.alreadyPaying = true
        window.pieceUntil++

        // window.balance.minus(amount)
        // window.needToPay += amount

        // if (window.channelIsOpened !== true) {
        //   if (window.openingInProgress) return
        //   window.openingInProgress = true
        // }

        if (!(wire.peerEthereumAddress in window.wires)) {
          window.wires[wire.peerEthereumAddress] = wire
        }
        // meta += wire.peerEthereumAddress + ''

        vynos.ready().then(wallet => {
          wallet.buy(receiver, amount, gateway, meta, undefined, window.piecePrice * window.pieces).then(result => {
            let secret = {meta: meta, token: result.token, price: window.piecePrice}

            // new Promise(function (resolve, reject) {
            //   wire.mc_payment_handshake.sendSecret(secret)
            // });
          })
        })
...
Torrent.prototype.pause = function () {
  ...
  var self = this
  //window.backupList = [...self.pieces]
  self.pieces.forEach(function (piece, index) {
    if (self.pieces[index]) {
      self.pieces[index].cancelRemaining()
    }
  })
  window.piecePayed = Math.max(window.pieceDownloaded, window.piecePayed)
  window.pieceDownloaded = 0
  const video = document.getElementById('main-video')

  video.pause()

  window.nowPlaying = false
  window.currentTime = video.currentTime != undefined ? video.currentTime : 0
  window.downloadedBytes = self.downloaded
  console.log('Paused when $'+window.piecePayed*window.piecePrice+' was payed')
  console.log('Paused at '+window.currentTime)
...
Torrent.prototype.resume = function () {
  ...
  this._debug('resume')
  var self = this
  //self.pieces = [...window.backupList]
  this.paused = false
  this._drain()

  window.resumeCallback(self)
}
